...
<details>
	<summary>장애회고</summary>

현상
```
좌석 예약 실패 발생. 예약 요청은 정상적으로 처리되었으나,
예약 이벤트가 발생한 후, 이벤트를 처리하는 카프카가 다운됨으로 인해 이벤트 전달이 실패함.
```

원인
```
카프카 다운으로 인해 예약 이벤트가 처리되지 못함. 
카프카는 예약 완료 후 후속 작업 실패가 발생함.
```
발생시간
```
8월 22일 2시
...
카프카 다운 시 로직 추가 필요:
    ...
    카프카 다운 상태에서도 중요한 예약 정보를 안전하게 처리할 수 있는 로직이 추가되어야 함. 
    이를 통해 예약 이벤트 실패 시에도 사용자 경험을 보호하고, 데이터의 일관성을 유지할 수 있도록 함.

서비스 가용성 저하:

    이벤트 전달 실패로 인해 예약 후속 작업 처리되지 않아 사용자가 혼란을 겪었을 가능성이 있음. 
    이는 고객 신뢰도에 부정적인 영향을 미칠 수 있음.
```
...
장애 모니터링 추가
    ...
    카프카 및 관련 이벤트 처리 시스템에 대한 실시간 모니터링을 강화함.
    장애 발생 시 빠르게 인지할 수 있는 알림 시스템을 구축해, MTTD를 줄이고, 대응 속도를 높임.
    예약 실패 시 즉시 알림이 생성되도록 하여 운영팀이 빠르게 대응할 수 있도록 함.
```

미드텀
```
카프카 장애 시 예약 데이터를 안전하게 처리하는 백업 메커니즘 구축:

    카프카가 다운되더라도, 예약 데이터를 안전하게 저장하고 후속 처리를 보장할 수 있는 백업 메커니즘 도입. 
    예를 들어, 이벤트 큐에 데이터를 임시 저장하거나, DB에 로그를 남겨 복구할 수 있도록 하는 방안을 고려함.

카프카 장애 시 자동 복구 프로세스 구축:

    카프카 다운 시 자동으로 복구하는 시스템을 구축하여, 서비스 중단 시간을 최소화함. 
    예를 들어, 자동 재시작 및 장애 복구 스크립트를 도입하여 MTTR을 단축할 수 있음.
```

롱텀
```
카프카와의 의존성을 줄이기 위한 아키텍처 개선:

    시스템이 카프카에 과도하게 의존하지 않도록 아키텍처를 개선함
    
고가용성(HA) 구성 도입:

    카프카 클러스터의 고가용성(HA) 구성을 통해, 단일 장애점(SPOF)을 제거함. 
    다중 브로커와 리플리케이션 설정을 강화하여, 브로커 중 하나가 다운되더라도 
    다른 브로커가 대체 역할을 수행할 수 있도록 함.
```
